...
{
    /// <summary>
    /// Interaction logic for ZJGG_DC.xaml
    /// </summary>
    public partial class ZJGG_DC : ArcGIS.Desktop.Framework.Controls.ProWindow
    {
        public ZJGG_DC()
        {
            InitializeComponent();
                        ...
                        using (RowCursor rowCursor = featureClass.Search())
                        {
                            while (rowCursor.MoveNext())
                            {
                                using (Feature feature = rowCursor.Current as Feature)
                                {
                                    ...
                                    string title = $"{count},{ft_mj},,{ft_mc},,,,{ft_yt},@ " + "\r\n";
                                    txt_all += title;
                                    ...
                                    for (int i = 0; i < mapPoints.Count; i++)
                                    {
                                        for (int j = 0; j < mapPoints[i].Count; j++)
                                        {
                                            ...
                                            {
                                                // 加入文本
                                                txt_all += $"{jFront}{ptIndex},{XX},{YY},,{i + 1},\r\n";
                                            }
                                            else
                                            {
                                                // 加入文本
